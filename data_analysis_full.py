# -*- coding: utf-8 -*-
"""
Created on Mon Mar 13 17:09:43 2023
...
s1=['Hydrogen_BAU','Hydrogen_1p5C','Hydrogen_BAU_full','Hydrogen_1p5C_full']
s=s1
...
scenario_axis=np.repeat(['REF_12','$1.5{^\circ}C_12$'],30)
scenario_axis=np.concatenate([scenario_axis,np.repeat(['REF_8760','$1.5{^\circ}C_8760$'],20)],axis=0)

scenario_axis_num=np.repeat([1,3],30)
scenario_axis_num=np.concatenate([scenario_axis_num,np.repeat([2,4],20)],axis=0)
...
for i in range(1):
    ...
    if i==0:
        ...
        fig.add_trace(go.Bar(x=x1, y=list(output_capacity_tmp.Storage), name='Storage', marker_color=storage_color,legendgroup='Storage'),row=i+1,col=1)
        ...
        fig.add_trace(go.Bar(x=x1, y=list(output_capacity_tmp.Storage), name='Storage', marker_color=storage_color,legendgroup='Storage',showlegend=False),row=i+1,col=1)
        ...
        fig.add_trace(go.Bar(x=x1, y=list(output_capacity_tmp.Hydrogen_fuel_cell), name='Hydrogen fuel cell', marker_color=turbine_color,legendgroup='Hydrogen fuel cell',showlegend=False),row=i+1,col=1)
    ...
    fig.add_trace(go.Bar(x=x1, y=list(output_generation_tmp.Storage), name='Storage', marker_color=storage_color,legendgroup='Storage',showlegend=False),row=i+1,col=2)
    ...
    fig.add_trace(go.Bar(x=x1, y=list(output_generation_tmp.Hydrogen_fuel_cell), name='Hydrogen_cell', marker_color=turbine_color,legendgroup='Hydorgen fuel cell',showlegend=False),row=i+1,col=2)
    #fig['layout']['yaxis'+str(i*2+2)].update(range=[0,15000],tickformat=',d')
    fig.update_yaxes(range=[-1000,20000],tickformat=',d',row=i+1,col=2)
    fig.add_annotation(xref='x domain',
                   yref='y domain',
                   x=0.01,
                   y=1.1,
...
fig.update_layout(barmode="relative", font = dict(size=12),
                  ...
                  legend=dict(yanchor="top",y=1,xanchor="left", x=1.00), margin=dict(l=10,r=10,b=10,t=10),
                  )
...
fig.show()
fig.write_image( 'H:/Hydrogen/mix_BAU.jpeg')
