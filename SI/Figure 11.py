...
for case in test_id:
    ...
    load_H2_tmp.loc[load_H2_tmp.zone=='West_Inner_Mongolia_H2','zone'] = 'Inner Mongolia'
    load_H2_tmp.loc[load_H2_tmp.zone=='East_Inner_Mongolia_H2','zone'] = 'Inner Mongolia'    
    ...
    West= capacity_province_tmp.loc[capacity_province_tmp.load_zone.isin(['West_Inner_Mongolia','West_Inner_Mongolia_H2']),:]
    East= capacity_province_tmp.loc[capacity_province_tmp.load_zone.isin(['East_Inner_Mongolia','West_Inner_Mongolia_H2']),:]
    ...
    capacity_province_tmp=capacity_province_tmp.drop(capacity_province_tmp[capacity_province_tmp['load_zone'].isin(['East_Inner_Mongolia','West_Inner_Mongolia','East_Inner_Mongolia_H2','West_Inner_Mongolia_H2'])].index,axis=0)
    ...
    generation_H2_tmp.loc[generation_H2_tmp.load_zone=='West_Inner_Mongolia_H2','load_zone'] = 'Inner Mongolia_H2'
    generation_H2_tmp.loc[generation_H2_tmp.load_zone=='East_Inner_Mongolia_H2','load_zone'] = 'Inner Mongolia_H2'  
    ...
    import_H2_tmp.loc[import_H2_tmp.load_zone=='West_Inner_Mongolia_H2','load_zone'] = 'Inner Mongolia_H2'
    import_H2_tmp.loc[import_H2_tmp.load_zone=='East_Inner_Mongolia_H2','load_zone'] = 'Inner Mongolia_H2'    
    ...
    transmission_H2.loc[transmission_H2.load_zone_from=='East_Inner_Mongolia_H2','load_zone_from']='Inner Mongolia'
    transmission_H2.loc[transmission_H2.load_zone_from=='West_Inner_Mongolia_H2','load_zone_from']='Inner Mongolia'
    transmission_H2.loc[transmission_H2.load_zone_to=='East_Inner_Mongolia_H2','load_zone_to']='Inner Mongolia'
    transmission_H2.loc[transmission_H2.load_zone_to=='West_Inner_Mongolia_H2','load_zone_to']='Inner Mongolia'
...
transmission_H2_expand = transmission_H2_province
...
transmission_ele_expand = transmission_province

#450

generation_ele_expand=generation_province.loc[:,['load_zone','power_mw']]
generation_hydrogen_expand=generation_H2_province.loc[:,['load_zone','H2_mw']]
generation_hydrogen_expand.load_zone = generation_hydrogen_expand.load_zone.str.split('_').str[0]
generation_hydrogen_expand = generation_hydrogen_expand .groupby(['load_zone'])['H2_mw'].sum().reset_index()
generation_hydrogen_expand.H2_mw.sum()

import_ele_expand=import_province.loc[:,['load_zone','net_imports_mw']]
import_hydrogen_expand=import_H2_province.loc[:,['load_zone','net_imports_H2_mw']]
import_hydrogen_expand.load_zone = import_hydrogen_expand.load_zone.str.split('_').str[0]
import_hydrogen_expand = import_hydrogen_expand.groupby(['load_zone'])['net_imports_H2_mw'].sum().reset_index()
...
ele_expand=load_province.loc[:,['zone','load_mw']]
H2_expand=load_H2_province.loc[:,['zone','H2_mw']]
H2_expand.zone = H2_expand.zone.str.split('_').str[0]
...
capacity_province.load_zone = capacity_province.load_zone.str.split('_')[:-1].str[0]
capacity_province = capacity_province .groupby(['load_zone','technology'])[['capacity_mw','capacity_mwh']].sum().reset_index()

capacity_storage_expand=capacity_province.pivot(index='load_zone',columns=['technology'],values='capacity_mwh').reset_index()
...
capacity_hydrogen_400=capacity_province
...
re_expand_load_H2.plot(ax=ax4, column='H2_mw',
                   norm = norm_direction1,
                   cmap=new_cmap,
                   #scheme='UserDefined', 
                   #classification_kwds={'bins': bins1},
...
missing_geom = re_storage_expand[re_storage_expand['Salt_cavern'].isna()]
missing_geom.plot(ax=ax9, color='lightgrey', edgecolor='grey', linewidth=0.25)
...
'''
#%%
fig.savefig('/Users/hy4174/Documents/Hydrogen/Figure/' + 'Fig4_decouple' + '.png', dpi=300, bbox_inches='tight')
