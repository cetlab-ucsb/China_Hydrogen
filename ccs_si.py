...
test_id=[
         ...
         'Hydrogen_400ppm_1period_36day_ccs_storage_80',
         'Hydrogen_400ppm_1period_36day_ccs_storage_biomass',
...
for case in test_id:
    ...
    cost_group=pd.concat([cost_group,cost_tmp])
    ...
    if ccs_capture_tmp.empty:
        ccs_capture_tmp=pd.DataFrame({
            ...
            'ccs_capture_tonne': 0},
            index=[case])
    carbon_capture=pd.concat([carbon_capture, ccs_capture_tmp])
    

    try:
        dispatch_tmp=pd.read_csv(ccs_commit_path)
        dispatch_tmp.ccs_power_mw =  dispatch_tmp.ccs_power_mw * dispatch_tmp.timepoint_weight/10**6
        dispatch_tmp=dispatch_tmp.groupby(['technology'])['ccs_power_mw'].sum().reset_index()
    except:
...
color=['#404040','#C0C0C0','#316589','#EA4855',
       ...
       '#8ac926','#254d32','#6ba542']
#%%
color_generation=['#404040','#C0C0C0','#316589','#EA4855',
       '#84BBD7','#2897C0','#F7AE39','#FADC65','#94d2bd','#0a9396','#d9ed92','#3f37c9',
       '#582f0e','#a68a64','#8ac926','#6ba542']
...
label=['Coal + CCS','Gas + CCS','Hydropower','Nuclear',
       ...
       'Electricity grid',' $H_{2}$ pipeline',' $CO_{2}$ pipeline','DAC','CCS storage','Biomass + BECCS']
...
fig.legend(reversed(handles[0:4]), reversed(label[0:4]),
           ...
           loc='center left', bbox_to_anchor=(0.98, 0.05),frameon=False
           )._legend_box.align = "left"


fig.legend(reversed(handles[4:7]), reversed(label[4:7]),
           title='Renewable energy',
           loc='center left', bbox_to_anchor=(0.98, 0.21),frameon=False
           )._legend_box.align = "left"
...
fig.legend(reversed(handles[9]), [label[9]],
           title='Power to gas',
           loc='center left', bbox_to_anchor=(0.98, 0.45),frameon=False
           )._legend_box.align = "left"

fig.legend(reversed(handles[10:12]), reversed(label[10:12]),
           title='Gas to power',
           loc='center left', bbox_to_anchor=(0.98, 0.55),frameon=False
           )._legend_box.align = "left"

fig.legend(reversed(handles[12:14]), reversed(label[12:14]),
           title='Hydrogen storage',
           loc='center left', bbox_to_anchor=(0.98, 0.67),frameon=False
           )._legend_box.align = "left"

fig.legend(reversed(handles[14:17]), reversed(label[14:17]),
           title='Transmission',
           loc='center left', bbox_to_anchor=(0.98, 0.79),
           frameon=False
           )._legend_box.align = "left"
...
ax4_1.set_ylabel('million tonne')
ax4_1.set_ylim([0,650/33.3])

ax5.set_ylabel('TWh')
ax5.set_xlabel('')
ax5.set_ylim([-5000,20000])
ax5.set_xticklabels(['REF','ZE', 'ZE + CCS + DAC','ZE + CCS + DAC + BECCS', 'ZE + Nuclear','ZE + Nuclear + High'])

ax6.set_ylabel('$/MWh')
ax6.set_xlabel('')
ax6.set_xticklabels(['REF','ZE', 'ZE + CCS + DAC','ZE + CCS + DAC + BECCS', 'ZE + Nuclear', 'ZE + Nuclear + High'])
...
#%%
fig.savefig('/Users/hy4174/Documents/Hydrogen/Figure/' + 'FigS_CCS' + '.png', dpi=300, bbox_inches='tight')
